...
export default async function AdminPhaseDetailPage({
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-12">
      ...
      <div className="mt-8 flex items-center justify-between">
        <h2 className="text-sm font-medium text-neutral-500">أسئلة الاختبار</h2>
        <Link href={`/admin/roadmaps/${id}/phases/${phase.id}/quiz-questions/new`} className="text-sm text-[#0F6E56]">
          + سؤال جديد
        </Link>
      </div>
      <div className="mt-3 flex flex-col gap-2">
        ...
        {phase.quizQuestions.map((question) => (
          <div key={question.id} className="rounded-lg border border-neutral-200 px-4 py-3">
            ...
            <div className="mt-2 flex items-center gap-3">
              <Link
                href={`/admin/roadmaps/${id}/phases/${phase.id}/quiz-questions/${question.id}/edit`}
                className="text-xs text-[#0F6E56]"
              >
                تعديل
              </Link>
